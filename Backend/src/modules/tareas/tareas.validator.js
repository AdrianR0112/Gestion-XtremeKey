...
function validatePayload(payload = {}, { isUpdate = false } = {}) {
  ...
  if (!isUpdate) {
    if (!clean.Tit_Tar || String(clean.Tit_Tar).trim() === '') {
      errors.push('Tit_Tar is required');
    }
  }
  ...
  clean.Des_Tar = normalizeOptionalString(clean.Des_Tar);

  const optionalIds = ['Id_Cli', 'Id_Ven'];
  ...
  if (clean.Fec_Com_Tar !== undefined) {
    if (clean.Est_Tar !== undefined && clean.Est_Tar !== 'completada') {
